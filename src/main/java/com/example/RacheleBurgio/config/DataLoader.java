...
public class DataLoader {
    ...
    CommandLineRunner initData(PizzaRepository pizzaRepo, DrinkRepository drinkRepo, ToppingRepository toppingRepo) {
        return args -> {
            Topping mozzarella = new Topping("Mozzarella", 1.50);
            Topping funghi = new Topping("Funghi", 2.00);
            toppingRepo.saveAll(List.of(mozzarella, funghi));

            Pizza margherita = new Pizza("Margherita", 8.00, List.of(mozzarella));
            Pizza funghiPizza = new Pizza("Funghi", 10.00, List.of(mozzarella, funghi));
            pizzaRepo.saveAll(List.of(margherita, funghiPizza));

            Drink cola = new Drink("Coca-Cola", 2.50);
            Drink birra = new Drink("Birra", 3.50);
